...
import { readableStreamToString, VisierObjectConfigurationError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new VisierObjectConfigurationError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new VisierObjectConfigurationError(e, e.response?.data)
                }
                if (innerError instanceof VisierObjectConfigurationError) {
                    // Got "VisierObjectConfigurationError" from the above try block
                    throw innerError;
                }
